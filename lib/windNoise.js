...
export function getWindNoiseTexture() {
  ...
  // 8-bit RGBA texture (NOT FloatType): iOS Safari + many lower-tier
  // mobile GPUs don't reliably support OES_texture_float_linear, so a
  // FloatType DataTexture with LinearFilter would either throw on
  // upload or silently render black there. UnsignedByteType has plenty
  // of precision for wind direction (we'd never notice the 1/256 step).
  const data = new Uint8Array(SIZE * SIZE * 4);
  for (let y = 0; y < SIZE; y++) {
    for (let x = 0; x < SIZE; x++) {
      const i = (y * SIZE + x) * 4;
      // Map fbm 0..1 → -1..1 (signed direction), then back to 0..1
      // for storage. Shader decodes with `rg * 2.0 - 1.0`.
      const wx = fbm(grid1, x, y) * 2 - 1;
      const wz = fbm(grid2, x, y) * 2 - 1;
      data[i + 0] = Math.round((wx * 0.5 + 0.5) * 255);
      data[i + 1] = Math.round((wz * 0.5 + 0.5) * 255);
      data[i + 2] = 0;
      data[i + 3] = 255;
    }
  }
